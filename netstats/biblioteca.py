...
def clusteringLocal(grafo, v):
    ...
    for i in range(len(adj)):
        for j in range(i+1, len(adj)):
            if adj[i] == adj[j]:
                continue
            if grafo.estan_unidos(adj[i], adj[j]):
                links+=1
            if grafo.estan_unidos(adj[j], adj[i]):
                links+=1
